...
if __name__=='__main__':
    ...
    # data has to have following shape:
    # [n_sampeles, n_classes, ...] 
    ...
    calib.Train( training_set, test_set, loss = 'nll_loss', epochs = 5, lr = 0.001, batch_size = 500, test_priod = 5, plot=False)
       
    model = V_ATTA(n_classes,n_augms)
    calib = Calibration(model, checkpoints='checkpoint')
    
    #print(test_set['labels'].shape,test_set['p0'].shape,test_set['p'].shape)
    
    test_set  = to_format_dict(test_set)
    prediction,_ = calib._test_epoch(1,test_set)
